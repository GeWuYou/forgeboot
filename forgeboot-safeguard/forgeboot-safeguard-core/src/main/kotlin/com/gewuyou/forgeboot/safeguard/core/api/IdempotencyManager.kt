...
interface IdempotencyManager {
     ...
     */
    operator fun get(key: Key): IdempotentRecord?
     ...
     */
    fun tryAcquirePending(key: Key, policy: IdempotentPolicy): Boolean
     ...
     * @param policy 幂等性策略，定义了操作的行为规则
     */
    fun saveSuccess(key: Key, record: IdempotentRecord, policy: IdempotentPolicy)
     ...
     * @param policy 幂等性策略，定义了操作的行为规则
     */
    fun updateStatus(key: Key, status: IdempotencyStatus, policy: IdempotentPolicy)
}
